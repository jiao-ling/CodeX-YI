...
const HexagramDataService = (function() {
    ...
    const HEXAGRAM_DATA_URL = 'data/hexagrams.json';
    ...
    async function init() {
        try {
            ...
            // 加载卦象数据
            hexagramSourceData = await loadHexagramData();
    ...
    async function loadHexagramData() {
        try {
            ...
            // 支持外部数据同时提供 bagua 和 hexagrams 字段
            if (data.bagua && typeof data.bagua === 'object') {
                baguaData = { ...baguaData, ...data.bagua };
            }

            if (data.hexagrams && typeof data.hexagrams === 'object') {
            ...
            return YizhiApp.utils.deepClone(fallbackHexagramsData);
        }
    }
