 ...
 * 占卜模块 - 管理铜钱投掷和卦象生成
 */
global.DivinationModule = (function() {
    // 私有变量
    ...
    async function saveToHistory() {
        ...
        try {
            ...
            const historyModule = await ensureHistoryModule();
            if (!historyModule) {
                YizhiApp.getModule('notification')?.show('error', '保存失败',
                    '未能加载历史记录模块，请稍后重试。', 4000);
                return;
            }
    ...
    async function ensureHistoryModule() {
        const existingHistory = YizhiApp.getModule('history');
        if (existingHistory) {
            return existingHistory;
        }

        if (global.ModuleLoader?.ensure) {
            try {
                return await global.ModuleLoader.ensure('history');
            } catch (error) {
                YizhiApp.errors.handle(error, 'Load History Module');
            }
        }
